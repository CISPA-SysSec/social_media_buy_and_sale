...
def load_embeddings(path_embeddings, path_data_dir, clustering_mode):
    ...
    if embeddings is None:
        embeddings = torch.empty(0) # Initialize as an empty tensor if no embeddings loaded
...
def load_embeddings_and_sentences(path_embeddings, path_data_dir, clustering_mode, also_titles=False):
    ...
    file_names = [os.path.join(root, file) for root, dirs, files in os.walk(complete_path) for file in files if file.endswith(".json.pt")]
